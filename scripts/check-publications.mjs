...
const enrichedArticles = await Promise.all(articles.map(enrichArticleAuthors));
const citationSvgUpdated = await updateCitationSvg(scholarData);
const newArticles = enrichedArticles
...
async function enrichArticleAuthors(article) {
  if (!needsAuthorExpansion(article)) {
    return article;
  }

  const expandedAuthors = await fetchExpandedAuthors(article);

  if (expandedAuthors.length === 0) {
    console.log(`Could not expand abbreviated author list for: ${article.title}`);
    return article;
  }

  return {
    ...article,
    authors: expandedAuthors.join(", "),
  };
}

function needsAuthorExpansion(article) {
  const authors = String(article.authors || "");

  return authors.includes("...") && !parseAuthors(authors).some(isPedro);
}

function hasAbbreviatedAuthors(authors) {
  return authors.some((author) => author.includes("..."));
}

async function fetchExpandedAuthors(article) {
  const scholarAuthors = await fetchScholarArticleAuthors(article);

  if (scholarAuthors.length > 0 && !hasAbbreviatedAuthors(scholarAuthors)) {
    return scholarAuthors;
  }

  const crossrefAuthors = await fetchCrossrefAuthors(article);
...
async function fetchScholarArticleAuthors(article) {
  ...
  url.searchParams.set("citation_id", article.citation_id);
  url.searchParams.set("api_key", SERPAPI_KEY);
  ...
  try {
    ...
    const data = await response.json();
    return parseAuthors(data?.article?.authors || data?.citation?.authors || data?.authors);
  } catch {
    return [];
  }
}

async function fetchCrossrefAuthors(article) {
  ...
  url.searchParams.set("select", "title,author,published-print,published-online,created");

  try {
    const response = await fetch(url, {
      headers: {
        "User-Agent": "personal-website-publication-check/1.0 (mailto:phrs16@gmail.com)",
      },
    });
    ...
    const match = items.find((item) => titlesMatch(title, item?.title?.[0]));

    if (!match || !Array.isArray(match.author)) {
      return [];
    }

    return match.author.map(formatCrossrefAuthor).filter(Boolean);
  } catch {
    return [];
  }
}

function titlesMatch(sourceTitle, candidateTitle) {
  const source = normalizeText(sourceTitle);
  const candidate = normalizeText(candidateTitle);

  return source && candidate && (source === candidate || source.includes(candidate) || candidate.includes(source));
}
...
async function updateCitationSvg(scholarData) {
  ...
  const metrics = getCitationMetrics(scholarData);
  const citationSvg = generateCitationSvg(graph, metrics);
  let currentSvg = "";
...
function getCitationMetrics(scholarData) {
  const table = Array.isArray(scholarData?.cited_by?.table) ? scholarData.cited_by.table : [];
  const metrics = {
    total: findMetricValue(table, ["citations", "cited by", "cites"]),
    hIndex: findMetricValue(table, ["h_index", "h-index", "h index"]),
  };

  return {
    total: metrics.total ?? scholarData?.cited_by?.citations,
    hIndex: metrics.hIndex ?? scholarData?.cited_by?.h_index,
  };
}

function findMetricValue(rows, labels) {
  for (const row of rows) {
    const entries = Object.entries(row || {});
    const label = normalizeText(entries.map(([key]) => key).join(" "));

    if (!labels.some((candidate) => label.includes(normalizeText(candidate)))) {
      continue;
    }

    const value = findFirstNumber(row);
...
function findFirstNumber(value) {
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const parsed = findFirstNumber(item);
...
function generateCitationSvg(graph, metrics = {}) {
  ...
  const metricsLabel = formatCitationMetrics(metrics);
...
${bars.map((bar, index) => sketchBar(bar, index, chart.top + chartHeight)).join("\n")}
  ...
  <g fill="#1f2937" font-family="Georgia, serif">
    <text x="270" y="24" font-size="16" text-anchor="middle" opacity=".58">Citations (Google Scholar)</text>
${yTicks.map((tick) => {
...
${bars.map((bar) => `    <text x="${round(bar.x + bar.width / 2)}" y="${chart.top + chartHeight + 20}" font-size="11" text-anchor="middle" opacity=".72">${bar.year}</text>`).join("\n")}
    <text x="279" y="258" font-size="16" text-anchor="middle" opacity=".58">${metricsLabel}</text>
  </g>
</svg>
`;
}

function formatCitationMetrics(metrics) {
  const total = Number.isFinite(Number(metrics.total)) ? Math.round(Number(metrics.total)) : 800;
  const hIndex = Number.isFinite(Number(metrics.hIndex)) ? Math.round(Number(metrics.hIndex)) : 15;

  return `(Total=${total.toLocaleString("en-US")}, h-index=${hIndex})`;
}
